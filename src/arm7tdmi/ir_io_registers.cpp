...
{
	if (size == Word) {
		LOG_ERROR("Word writes should not reach ir io registers")
		exit(-1);
	}
	...
	if (seq != FREE) {
		if (address == IF || address == IF + 1) {
			...
			LOG_TRACE("IRQ Acknowledge {:X} was {:X} @ {:X}", value, curIF, address)
			value = curIF & ~value;
			LOG_TRACE("IRQ Acknowledge now {:X} @ {:X}", value, address)
	...
	if (BIT_RANGE(address, 0, 11) == 0x800) {
		LOG_ERROR("Internal Memory Control not implemented")
		exit(-1);
	}
}
}
