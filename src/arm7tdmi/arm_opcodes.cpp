	...
	bool regProvidedAmount)
{
	...
	{
		LOG_ERROR("CPU::Shift invalid parameters")
		break;
	}
	}
}
...
Op CPU::ArmOperation(OpCode opcode)
{
	...
	case 0b11: // CoProc and SWI
	{
		...
		if ((swiMask & opcode) == swiMask) {
			...
			LOG_ERROR("Attempting CoProc???")
			return ArmUndefined_P();
		}
	}
	default: {
		LOG_ERROR("This should be impossible, non matching opcode")
		exit(-1);
	}
	}
}
...
void CPU::ArmMRS(bool Ps, U8 Rd)
{
	...
	if (Ps) {
		...
		LOG_TRACE("	MRS SPSR {:X}", dest)
	} else {
		dest = registers.CPSR.ToU32();
		LOG_TRACE("	MRS CPSR {:X}", dest)
	}
}
...
void CPU::ArmMSR(bool I, bool Pd, bool flagsOnly, U16 source)
{
	...
	if (!flagsOnly) {
		if (Pd) {
			...
			LOG_TRACE("	MSR SPSR {:X}", value)
		} else {
			registers.CPSR.FromU32(value);
			LOG_TRACE("	MSR CPSR {:X}", value)
		...
		if (Pd) {
			...
			LOG_TRACE("	MSR SPSR_f {:X}", value)
		} else {
			registers.CPSR.FlagsFromU4(value);
			LOG_TRACE("	MSR CPSR_f {:X}", value)
		}
	}
}
...
void CPU::DataProcessing(U32 I, U32 OpCode, U32 S, U32 Rn, U32 Rd, U32 Op2, bool Adr)
{
	...
	case DPOps::AND: {
		dest = Op1Val & Op2Val;
		LOG_TRACE("	AND {:X} {:X} {:X}", Op1Val, Op2Val, dest)
		break;
	}

	case DPOps::EOR: {
		dest = Op1Val ^ Op2Val;
		LOG_TRACE("	EOR {:X} {:X} {:X}", Op1Val, Op2Val, dest)
		break;
	}
	...
	case DPOps::SUB: {
		...
		LOG_TRACE("	SUB {:X} {:X} {:X}", Op1Val, Op2Val, dest)
		carry = Op1Val >= Op2Val;
	...
	case DPOps::RSB: {
		...
		dest = (uint32_t)result;
		LOG_TRACE("	RSB {:X} {:X} {:X}", Op1Val, Op2Val, dest)
		carry = Op2Val >= Op1Val;
	...
	case DPOps::ADD: {
		auto result = (std::uint64_t)Op1Val + Op2Val;
		dest = (uint32_t)result;
		LOG_TRACE("	ADD {:X} {:X} {:X}", Op1Val, Op2Val, dest)
		carry = result >> 32;
		auto overflow = (~(Op1Val ^ Op2Val) & (Op1Val ^ dest)) >> 31;
		registers.CPSR.v = overflow;
		break;
	...
	case DPOps::ADC: {
		...
		dest = (uint32_t)result;
		LOG_TRACE("	ADC {:X} {:X} {:X} {:X}", Op1Val, Op2Val,
			carry, dest);
		auto overflow = ((~(Op1Val ^ Op2Val) & ((Op1Val + Op2Val) ^ Op2Val)) ^ (~((Op1Val + Op2Val) ^ carry) & (dest ^ carry))) >> 31;
		carry = result >> 32;
		registers.CPSR.v = overflow;
		break;
	}
	...
	case DPOps::SBC: {
		...
		LOG_TRACE("	SBC {:X} {:X} {:X} {:X}", Op1Val, Op2Val,
			Op3Val, dest);
	...
	case DPOps::RSC: {
		...
		LOG_TRACE("	RSC {:X} {:X} {:X} {:X}", Op1Val, Op2Val,
			Op3Val, dest);
	...
	case DPOps::TST: {
		...
		LOG_TRACE("	TST {:X} {:X} {:X}", Op1Val, Op2Val,
			result);
	...
	case DPOps::TEQ: {
		auto result = Op1Val ^ Op2Val;
		LOG_TRACE("	TEQ {:X} {:X} {:X}", Op1Val, Op2Val,
			result);
		SetFlags(1, result, carry);
		S = 0;
		break;
	...
	case DPOps::CMP: {
		auto result = (std::uint64_t)Op1Val - Op2Val;
		carry = Op1Val >= Op2Val;
		LOG_TRACE("	CMP {:X} {:X} {:X}", Op1Val, Op2Val,
			result);
		SetFlags(1, result, carry);
	...
	case DPOps::CMN: {
		...
		LOG_TRACE("	CMN {:X} {:X} {:X}", Op1Val, Op2Val,
			result);
		carry = resultBig >> 32;
		SetFlags(1, result, carry);
	...
	case DPOps::ORR: {
		dest = Op1Val | Op2Val;
		LOG_TRACE("	ORR {:X} {:X} {:X}", Op1Val, Op2Val, dest)
		break;
	}

	case DPOps::MOV: {
		dest = Op2Val;
		LOG_TRACE("	MOV {:X}", dest)
		break;
	}

	case DPOps::BIC: {
		dest = Op1Val & ~Op2Val;
		LOG_TRACE("	BIC {:X} {:X} {:X}", Op1Val, Op2Val, dest)
		break;
	}

	case DPOps::MVN: {
		dest = ~Op2Val;
		LOG_TRACE("	MVN {:X}", dest)
		break;
	}
...
void CPU::ArmMultiply(U32 A, U32 S, U32 Rd, U32 Rn, U32 Rs, U32 Rm)
{
	...
	if (Rd == Rm) {
		LOG_ERROR("Invalid Arm Mult Rd == Rm")
		return;
	}

	if (Rm == 15 || Rs == 15 || Rn == 15 || Rd == 15) {

		LOG_ERROR("Invalid Arm Mult Using PC")
		return;
	}
	...
	U32 Rm)
{
	...
	if (RdLo == Rm || RdHi == Rm || RdLo == RdHi) {
		LOG_ERROR("Invalid Arm Mult Long Rd == Rm")
		return;
	}

	if (Rm == 15 || Rs == 15 || RdHi == 15 || RdLo == 15) {
		LOG_ERROR("Invalid Arm Mult Long Using PC")
		return;
	}
...
void CPU::ArmSingleDataSwap(U32 B, U32 Rn, U32 Rd, U32 Rm)
{
	if (Rm == 15 || Rn == 15 || Rd == 15) {
		LOG_ERROR("Invalid Arm SDS Using PC")
		return;
	}
...
void CPU::ArmBranchAndExchange(U32 Rn)
{
	...
	LOG_TRACE("BRANCHEXCHANGE {:X}", val)
	...
	U32 S, U32 H, U32 Rm)
{
	...
	LOG_TRACE("HDT Reg Offset {:X}", Offset)
	...
	U32 H, U32 Offset)
{
	...
	auto memAddr = base;
	LOG_TRACE("HDT memAddr {:X} baseOffset {:X}", memAddr,
		baseOffset);
	...
	if (L) // LD
	{
		...
		if (H) // HalfWord
		{
			...
			if (wordBoundaryOffset) {
				LOG_WARN("half word boundary offset logic potentially wrong")
			...
			if (S) // Signed
			{
				...
				if (destReg >> 15) {
					...
					LOG_WARN("signed half word offset logic potentially wrong", destReg)
		...
		} else // Byte
		{
			if (S) {
				...
				LOG_ERROR("Not allowed to do unsigned byte transfer using HDT")
			}
		}
	} else // STR
	{
...
{
	LOG_TRACE("SDT I{} P{} U{} B{} W{} L{} Rn{} Rd{} Offset{}", I,
	...
	if (L) {
		...
		if (B) {
			...
			if (wordBoundaryOffset) {
				...
				LOG_TRACE("Word Boundary Offset {} for value {:X}",
					wordBoundaryOffset, value);
				Shift(value, wordBoundaryOffset * 8, ROR, emptyCarry, false);
			}

			LOG_TRACE("R{:X} <- {:X}", Rd, value)
			destReg = value;
			if (Rd == 15) {
				PipelineFlush();
			}
...
{
	Offset <<= 2;
	LOG_TRACE("BRANCH L{:X} Offset:{:X}", L, Offset)
	...
	if (L) {
		registers.get(Register::R14) = (registers.get(Register::R15) - 4) & ~NBIT_MASK(2);
	}
