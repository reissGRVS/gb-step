...
void PPU::DrawObject(ObjAttributes objAttrs) {
  auto [spriteWidth, spriteHeight] =
      OBJ_DIMENSIONS[objAttrs.attr1.b.objSize][objAttrs.attr0.b.objShape];
  ...
  for (std::uint16_t tileX = 0; tileX < spriteWidth; tileX++) {
	for (std::uint16_t tileY = 0; tileY < spriteHeight; tileY++) {
	  ...
	  std::uint16_t x =
	      objAttrs.attr1.b.xCoord + TILE_PIXEL_WIDTH * actualTileX;
	  std::uint16_t y =
	      objAttrs.attr0.b.yCoord + TILE_PIXEL_HEIGHT * actualTileY;
...
void PPU::DrawTile(const TileInfo& info) {
  const auto& [startX, startY, tileNumber, colorDepth, tileDataBase,
               verticalFlip, horizontalFlip, paletteNumber, priority] = info;
