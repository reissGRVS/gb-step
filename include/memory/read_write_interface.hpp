	...
	U32 address, const AccessSize& size)
{
	...
	case AccessSize::Byte: {
		if (address >= arr.size()) {
			LOG_ERROR("Out of bounds byte read @ {:X}", address)
			exit(20);
		}
		return arr[address];
	}
	case AccessSize::Half: {
		if (address + 1 >= arr.size()) {
			LOG_ERROR("Out of bounds half read @ {:X}", address)
			exit(20);
		}
	...
	case AccessSize::Word: {
		if (address + 3 >= arr.size()) {
			LOG_ERROR("Out of bounds word read @ {:X}", address)
			exit(20);
		}
		return (arr[address] + (arr[address + 1] << 8)
	...
	LOG_ERROR("Read to size no match")
	exit(1);
}
	...
	U32 address, U32 value, const AccessSize& size)
{
	...
	case AccessSize::Byte: {
		if (address >= arr.size()) {
			LOG_ERROR("Out of bounds byte write @ {:X}", address)
			exit(20);
		}
	...
	case AccessSize::Half: {
		if (address + 1 >= arr.size()) {
			LOG_ERROR("Out of bounds half write @ {:X}", address)
			exit(20);
		}
		arr[address] = (U8)value;
		value >>= 8;
	...
	case AccessSize::Word: {
		if (address + 3 >= arr.size()) {
			LOG_ERROR("Out of bounds word write @ {:X}", address)
			exit(20);
		}
